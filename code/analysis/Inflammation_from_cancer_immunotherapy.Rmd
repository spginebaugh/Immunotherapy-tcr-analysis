...
## TCR analysis TODO: figures 4 and S5
...
expansion_ratio <- cd_all_meta %>%
  ...
  summarize(
    ...
    ratio = expanded_unique / single_count
  )
...
## TODO: Fix the ordering and colors of the plots
...
ggplot(expansion_ratio, aes(x = patient_group , y = expanded_unique, fill = celltype  )) +
  ...
  theme_prism() +
  theme(axis.text.x = element_text(color = colors, angle = 90, vjust = 0.5, hjust = 1)) +
  ylab("# unique expanded clonotypes per patient") +
  xlab("")
...
ggplot(expansion_ratio, aes(x = patient_group , y = ratio, fill = celltype)) +
  ...
  theme_prism() +
  theme(axis.text.x = element_text(color = colors, angle = 90, vjust = 0.5, hjust = 1)) +
  ylab("Number of expanded clonotypes per patient") +
  xlab("")
...
### CD4+ TCR analysis
```{r CD4_tcr_1, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 11, fig.height = 5}

DimPlot(cd4_merge, group.by = "cloneSize")
clonalOverlay(cd4_merge, reduction = "umap", cutpoint = 1, bins = 10, facet.by = "patient_group")
clonalNetwork(cd4_merge, reduction = "umap", group.by = "annotation_level3",
              filter.clones = NULL, filter.identity = "Th1_effector", cloneCall = "strict")
```

```{r cd4_tcr_2, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 11, fig.height = 5}
clonalOverlay(cd4_merge, 
              reduction = "umap", 
              cutpoint = 1, 
              bins = 10, 
              facet.by = "patient_group") + 
...
```{r cd4_tcr_3, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 11, fig.height = 5}
cd4_merge2 <- cd4_merge

cd4_merge2$cdrs3_pat <- paste(cd4_merge2$CTaa, cd4_merge2$sample, sep = "_")

freqs <- (table(cd4_merge2$cdrs3_pat)/sum(!is.na(cd4_merge2$cdrs3_pat))) %>% data.frame()
sorted <- freqs$Freq
names(sorted) <- freqs$Var1
sorted <- sort(sorted, decreasing = TRUE)
largest_clones <- head(sorted, 6)

cells <- which(cd4_merge2$cdrs3_pat %in% names(largest_clones))
DimPlot(cd4_merge, reduction = "umap", cells.highlight = cells) +NoLegend()
...
```{r cd4_tcr_4, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 11, fig.height = 5}
combined.TCR <- list()
donor_vec <- unique(combined_tcr$donor_id)
donor_vec <- donor_vec[order(donor_vec)]
