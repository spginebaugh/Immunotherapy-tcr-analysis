...
```{r allcell_umap_celltype, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 11, fig.height = 5,}
DimPlot(sc_all, group.by = c("annotation_level1", "patient_group"), label = TRUE, raster = TRUE)
...
```{r allcell_cell_prop, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 11, fig.height = 5,}
...
Next, we investigate the T-cells in greater detail. We see that the T-cells generally separate into CD8 and CD4 subtypes. However, it isn't perfect -- there are some clusters (e.g. cluster 10) which contain both CD4 & CD8 cells, and there are some clusters which contain neither CD4 or CD8 cells. 
...
To remedy this, we instead use [Azimuth](https://www.cell.com/cell/fulltext/S0092-8674(21)00583-3) to annotate the cell types. We found that this worked very well for the initial labeling of CD4/CD8/other celltypes, but did a poor job of capturing heterogeneity in sub-subtypes. So, we next separated the CD4 and CD8 T-cells based on the Azimuth level 1 annotation, and studied them separately.

```{r Azimuth_umap, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 11, fig.height = 5,}
# TODO: fix with all tcells
DimPlot(seurat, group.by = c("azimuth_celltype_1","azimuth_celltype_2"), label = TRUE, raster = FALSE)
```

```{r Azimuth_cell_prop, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 11, fig.height = 5,}
ggplot(seurat@meta.data, aes(x = donor_id, fill = azimuth_celltype_1)) +
  geom_bar(position = "fill") +
  theme_prism()
```
